...
; (function ($) {
    ...
    function _preloaderStop() {
        $('div.jquery-preloader').each(function () {
            clearTimeout($(this).data('interval'));
            $(this).fadeOut(1500, function () { // Callback después del fadeOut
                $(this).remove(); // Remueve el elemento después de la animación
            });
        });
        $('div.jquery-preloader-overlay').slideUp(800, function () {
            $(this).remove(); // Remueve la capa de fondo después de la animación
        });
    }
