   ...
   */
    configurarResizeObserver() {
        ...
        this.resizeObserver = new ResizeObserver(entries => {
            for (let entry of entries) {
                ...
                console.log(`Ancho de #${this.idContTable} cambió a: ${width}px`);
                this.syncScrollWidth();
            }
        });
        ...
        console.log(`ResizeObserver configurado para #${this.idContTable}`);
    }
    ...
    destroy() {
        ...
        if (this.resizeObserver) {
            ...
            console.log(`ResizeObserver desconectado para #${this.idContTable}`);
        }

        $scrollTop.remove();

        console.log(`TopScrollTable destruido: ${this.idScroll}`);
    }
...
class TopScrollManager {
    ...
    crear(idTable, idContTable, idScroll = null) {
        ...
        if (topScroll.createScroll() && topScroll.initScroll()) {
            ...
            console.log(`✅ TopScrollTable registrado: ${idScroll}`);
            return topScroll;
        } else {
    ...
    destruir(idScroll) {
        ...
        if (instancia) {
            ...
            console.log(`TopScrollTable eliminado: ${idScroll}`);
            return true;
        }
        return false;
    }
    ...
    destruirTodos() {
        this.instancias.forEach((instancia, id) => {
            instancia.destroy();
            console.log(`TopScrollTable eliminado: ${id}`);
        });
...
/*
// EN TU VISTA DE MANTENIMIENTOS
$(document).ready(function() {
    // Crear scroll para tabla de mantenimientos
    TopScrollManager.crear(
        "tablaMantenimientosRango",
        "ManntoScroll",
        "scrollMannto"
    );
});

// EN TU VISTA DE SOLICITUDES
$(document).ready(function() {
    // Crear scroll para tabla de solicitudes
    TopScrollManager.crear(
        "tablaSolicitudesRefacciones",
        "SolicitudScroll",
        "scrollSolicitud"
    );
});

// REFRESCAR DESPUÉS DE AJAX/DATATABLE
$('#tablaMantenimientosRango').on('draw.dt', function() {
    TopScrollManager.refrescar("scrollMannto");
});

// DESTRUIR AL SALIR DE LA VISTA
$(window).on('beforeunload', function() {
    TopScrollManager.destruirTodos();
});

// O DESTRUIR SOLO UNO
TopScrollManager.destruir("scrollMannto");

// VER TODAS LAS INSTANCIAS ACTIVAS
console.log(TopScrollManager.listar());
*/